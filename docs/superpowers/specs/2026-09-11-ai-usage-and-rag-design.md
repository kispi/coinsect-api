...
행 수는 피크 기준 하루 3~4천이다(방송인당 프레임 3장 x 5분 주기). 처음 적었던 "하루 수백
행"은 프레임을 한 장으로 세어 열 배 어긋난 값이었다 — 2026-09-11 최종 검토에서 잡혔다.
90일이면 35만 행, 인덱스까지 100MB 안쪽이라 이 상자에서 여전히 문제가 아니다. 그래도
지우는 이유는 용량이 아니라, 되짚을 일이 없는 데이터를 영원히 들고 있을 이유가 없어서다.
...
**임베딩의 토큰 수는 생성 호출과 다르게 온다.** `embedContent` 응답에는
`usageMetadata`가 없고 `metadata.billableCharacterCount`가 있다. 있으면 그걸 쓰고,
없으면 글자 수에서 추정한다. 추정한 행은 그 사실을 알 수 있어야 하므로 `error` 칸이
아니라 `model` 옆에 붙는 별도 표시 없이 **추정 규칙을 코드 한 곳에만 두고** 주석에
근거를 적는다. 배치로 묶어 보내면 한 요청이 여러 청크를 담으므로 **행은 요청당 하나**다.
청크마다 행을 남기면 호출 수가 실제와 어긋난다.
...
1. **글을 쓰거나 고친 직후.** 잡을 등록하고 배수를 그 자리에서 깨운다. 응답을 기다리지
   않으므로 글쓰기가 느려지지 않고, 1~2초 뒤면 벡터 경로에도 올라온다.
2. **cron 5분 주기.** 놓친 잡을 줍는 그물이다. 서버가 재시작되며 날아간 호출, 임베딩이
   실패했던 잡이 여기 걸린다.

cron만 두면 방금 쓴 글이 최대 한 주기 동안 벡터 경로에 없다. 키워드 경로가 `posts`
원본을 보므로 검색 자체는 즉시 되지만, **본문에 그 단어가 없고 의미로만 관련된 글**은
그 사이 못 찾는다. 즉시 깨우기가 그 틈을 지운다.

**배수는 한 번에 하나만 돈다.** 글이 연달아 올라오면 같은 잡을 둘이 잡아 임베딩 비용이
두 배가 된다. `core/cache`의 `hSetNX`로 잠근다. 먼저 읽고 없으면 쓰는 방식은 동시에 들어온
둘이 모두 통과하므로 안 된다. 해시 필드에는 개별 만료가 없어서, 값에 시작 시각을 적어두고
5분이 지난 잠금은 죽은 프로세스가 남긴 것으로 보고 뺏는다. `desktop_jobs.ts`가 잡을 잡는
방식과 같다. Redis를 끄면 메모리 구현으로 떨어지는데 `hSetNX`의 계약이 그쪽에서도 같아서
개발과 배포의 동작이 갈리지 않는다.
...
4. 청크별 해시로 `embedding_cache`를 먼저 본다. **없는 것만** 임베딩 API를 친다.
   **여러 청크를 한 요청에 묶는다** — `embedContent`의 `contents`는 배열을 받고
   `embeddings`를 넣은 순서 그대로 돌려준다. 하나씩 치면 6,000청크가 6,000요청이지만
   100개씩 묶으면 60요청이다. 배치 크기는 상수로 두고 **구현 때 실제 호출로 상한을
...
그건 API가 느려서가 아니라 우리가 정한 페이싱 때문이다. 직접 돌면 배치 임베딩이 병목이라
수십 분 안이다. **실제 소요는 백필 때 재서 여기에 숫자로 남긴다.**
...
| 배수 잠금 | 동시에 두 번 깨워도 하나만 돌고, 5분 지난 잠금은 뺏기는지 |
