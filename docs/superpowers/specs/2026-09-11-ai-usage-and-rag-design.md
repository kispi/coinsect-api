...
**자기가 배수를 반복해 돈다.** cron에 맡기면 5분에 20건이라 1,713건에 일곱 시간이 걸린다.
직접 돌면 임베딩 API가 병목이라 20~30분이다. 같은 잠금을 쓰므로 cron이나 글쓰기가 끼어들어
같은 잡을 두 번 처리하지 않는다. 중간에 죽어도 잡이 pending으로 남아 이어서 돈다.
