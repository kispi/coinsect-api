...
**thinking을 끄는 것이 이번에 가장 큰 이득이었다 (2026-09-09).**

운영은 `thinkingConfig`를 아예 주지 않고 있었다. 그러면 이 모델은 호출당 2,600토큰씩
생각하고, 그게 전부 출력 단가로 과금된다. 벤치는 `thinkingBudget: 0`을 주고 재고 있어서
운영 실제 비용을 낮게 잡고 있었다.

| 설정 | 점수 | 판독 불가 | 평균 지연 | thinking/호출 | 월 |
|---|---|---|---|---|---|
| `thinkingConfig` 미전달 (당시 운영) | 44/48 | 1/12 | 11,658ms | 2,651 | $80.09 |
| **`thinkingBudget: 0`** (현재) | **48/48** | 0/12 | **4,249ms** | 420 | **$21.49** |

정확도가 오히려 올라가고 2.7배 빨라진다. 트레이드오프가 없어서 바로 넣었다.

`thinkingLevel`(3.x의 새 파라미터)은 답이 아니었다. `MINIMAL`은 이 모델이
400 `Thinking level MINIMAL is not supported for this model`로 거부하고, `LOW`는 안 준
것과 차이가 없다. **`thinkingBudget`이 맞는 손잡이다.**

`thinkingBudget: 0`을 줘도 어려운 프레임에서는 400토큰쯤 생각한다. 완전히 0이 되지는
않지만 2,651에서 420이면 충분하다.
