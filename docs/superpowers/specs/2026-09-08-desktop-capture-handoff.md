...
픽스처 3장 x 4개 항목 = 12점. `thinkingBudget=0`으로 측정.

| 모델 | 점수 | 평균 지연 | 호출당 토큰 (입력/과금출력) | 호출당 비용 |
|---|---|---|---|---|
| `gemini-3.8-flash` (현재) | **12/12** | 5,949ms | 1,480 / 778 (thinking 724 포함) | $0.00403 |
| `gemini-3.5-flash` | 10/12 | 1,615ms | 1,480 / 55 | $0.00271 |
| `gemini-3.5-flash-lite` | 8/12 | 1,856ms | 1,480 / 55 | $0.00058 |
| `gemini-2.5-flash-lite` | 5/12 | 3,829ms | 653 / 55 | $0.00009 |

알아낸 것:

- **`thinkingBudget: 0`은 `gemini-3.8-flash`에서 지켜지지 않는다.** 호출마다 thinking 토큰이
  0~1,847로 들쭉날쭉하게 나온다. 그리고 이 thinking이 과금출력 778 중 724를 차지한다.
  **3.8-flash 비용의 대부분이 thinking이다.** 정말 끌 수 있다면 월 $28 → $9로 내려간다.
- **`gemini-3.5-flash-lite`는 `thinkingConfig` 자체를 400 INVALID_ARGUMENT로 거부한다.**
  첫 측정에서 이것 때문에 0/12로 찍혔다. 품질이 아니라 설정 오류였다. 도구가 이제 빼고 재시도한다.
- 싼 모델들이 틀리는 지점은 `soxl-full.jpg`에서 **진입가와 청산가를 맞바꾸는 것**이다.
  값이 그럴듯해 보여서 사람이 훑고 넘길 위험이 있는 종류의 오류다.
- **표본이 작다.** 픽스처 3장이고 재실행 간 점수가 흔들린다(3.5-flash가 9/12 → 10/12).
  12점 중 1~2점 차이는 유의미하지 않다고 봐야 한다.

### 2. 비용 — 실측 (2026-09-09)

단가는 2026-09-09 ai.google.dev/gemini-api/docs/pricing 기준.

| 모델 | 월 (피크 3명 라이브) | 월 (5명 상시) |
|---|---|---|
| `gemini-3.8-flash` | **$28.27** | $54.37 |
| `gemini-3.5-flash` | $19.06 | $36.65 |
| `gemini-3.5-flash-lite` | $4.08 | $7.85 |
| `gemini-2.5-flash-lite` | $0.61 | $1.18 |

호출 수 가정: 피크(21~3시) 6시간 x 5분 = 72바퀴 + 비피크 18시간 x 1시간 = 18바퀴 = 하루 90바퀴.
한 바퀴에 라이브인 대상 수만큼 호출하므로 피크 3명/비피크 1명이면 하루 234회, 월 약 7,020회다.
가정이 틀리면 위 표의 '호출당 비용'에 실제 호출 수를 곱해 다시 계산할 것.

**`gemini-3.8-flash`는 2027-01-01에 단가가 두 배가 된다 → 월 $56.55.**

판단:

- 정확도만 보면 3.8-flash가 유일하게 전부 맞혔고, 실전에서도 두 번(2026-09-08 4/4,
  2026-09-09 KORU 4/4) 맞았다. 지금 바꿀 근거는 없다.
- 비용을 줄여야 하면 `gemini-3.5-flash-lite`가 월 $4로 후보다. 다만 **§3의 정확도 계층이
  먼저다.** 진입/청산을 맞바꾸는 오류를 잡아줄 장치 없이 갈아타면 안 된다.
- 그 전에 3.8-flash의 thinking을 실제로 끄는 방법을 찾는 쪽이 이득이 크다.
  정확도를 그대로 두고 월 $28 → $9다.
