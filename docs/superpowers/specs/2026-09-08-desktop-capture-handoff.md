...
**결론: `gemini-3.8-flash` 유지.** flash-lite로 내려보려다 접었다.

`legible`을 넣고 **운영 코드 경로 그대로** 픽스처당 3회씩 재보니 갈렸다.

| 모델 | btc-full | soxl-full | soxl-crop-legible |
|---|---|---|---|
| `gemini-3.5-flash-lite` | 읽음 3/3 | **불가 0/3** | **불가 0/3** |
| `gemini-3.8-flash` | 읽음 3/3 | 읽음 3/3 | 읽음 3/3 |

flash-lite는 BTC 화면은 읽는데 **알트코인 화면을 통째로 판독 불가로 넘긴다.**
방송인들이 실제로 만지는 게 알트코인이라(2026-09-09 박호두 KORUUSDT) 월 $24를
아끼는 게 아니라 자동화를 수동 입력으로 바꾸는 셈이 된다.

원인은 프롬프트가 BTC 전용이라는 데 있어 보인다 — "size는 보통 1~100 BTC",
"비트코인은 다섯 자리" 같은 힌트가 SOXL(913.55주 / $138)과 정면으로 어긋난다.
힌트를 코인 일반으로 바꿔 재봤지만 픽스처 3장으로는 신뢰구간이 안 나온다.
**싼 모델로 가려면 픽스처를 20~30장으로 늘리는 것이 먼저다.**

남은 비용 카드:

- **3.8-flash의 thinking을 실제로 끄기.** 과금출력 778 중 724가 thinking이다.
  끌 수만 있으면 정확도 그대로 월 $28 → $9다. `thinkingBudget: 0`은 무시된다.
- §3의 정확도 계층을 먼저 세우고 나면 싼 모델을 다시 시도할 수 있다.

### 3. 정확도 계층 (선행 스펙 §4/§5)

**`legible` 신고는 2026-09-09에 들어갔다.** 남은 것은 `responseSchema`(지금은 프롬프트로만
스키마를 지시한다), 프레임 합의, 손익 역산이다.
