...
`.env`의 `POSITION_AUTO_APPROVE=yes`로 켠다. 켜면 판독을 슬랙으로 묻지 않고 바로
반영하고 결과 한 줄(`✅ 자동 승인됨 — …`)만 알린다. 기본은 꺼짐.

- **데스크톱 제보에만 적용된다.** 유저 제보까지 자동 반영하면 누구나 임의의 포지션을
  전체 푸시로 내보낼 수 있다
- **판독 실패(0개 읽음)는 자동승인 대상이 아니다.** 읽은 것이 없으면 지울 근거도 없고
  반영할 것도 없다. 스샷을 보고 손으로 넣으라는 제보가 그대로 나간다
- 승인 대기가 없으므로 제보함에 넣지 않고 **스샷도 S3에 올리지 않는다.** 승인 시점에
  지워질 파일이고, 무엇을 읽었는지는 결과 한 줄에 수치로 남는다
- 화면에서 사라진 계약 정리(unseen)도 함께 적용된다

**트레이드오프:** 사람이 스샷과 대조하는 층이 사라진다. 오인식이 그대로 유저에게
푸시된다. 켜기 전에 §1의 판독 정확도를 다시 확인할 것. 틀린 값이 나가도 어드민에서
고치면 되지만, 푸시는 되돌릴 수 없다.
