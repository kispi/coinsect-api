...
const search = {
  ...
  related: async ({ postId, boardId, limit = 5, minScore = RELATED_MIN_SCORE }: {
    ...
    // 접기 전에 얼마나 깊이 뽑을 것인가. limit의 배수로는 턱없이 부족하다.
    //
    // 2026-09-12 프로덕션에서 한 글(id 60)로 실측했다. 컷오프를 넘는 청크가 240행,
    // 그 안의 작성자가 11명인데 가까운 쪽부터 세면 이렇게 나온다.
    //
    //   상위 50행    작성자 3명
    //   상위 150행   작성자 4명
    //   상위 300행   작성자 11명
    //
    // 한 작성자가 이웃 수십 개를 연달아 차지하기 때문이다. 처음에 limit * 10으로
    // 뒀더니 다섯 건을 청했는데 세 건이 돌아왔다. 깊이는 limit이 아니라 '한 작성자가
    // 연달아 차지할 수 있는 길이'에 맞춰야 한다.
    //
    // 코퍼스가 커지면 이 값도 다시 재야 한다. 청크가 6,106개인 지금은 300행을 훑는
    // 비용이 무시할 만하다.
    const depth = Math.max(limit * 60, 300)
